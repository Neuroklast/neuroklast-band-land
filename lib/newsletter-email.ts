...
const DEFAULT_FROM = 'noreply@neuroklast.net'
...
export async function sendConfirmationEmail(email: string, confirmUrl: string): Promise<boolean> {
  const html = `
    <p>Please confirm your subscription to the Neuroklast mailing list.</p>
...
export async function sendWelcomeEmail(email: string, unsubscribeUrl: string): Promise<boolean> {
  const html = `
    <p>You're subscribed to the Neuroklast mailing list.</p>
  ...
  return sendResendEmail({
    from: DEFAULT_FROM,
    to: [email],
    subject: 'Welcome to the Neuroklast mailing list',
    html,
  })
}
...
export async function sendUnsubscribeConfirmationEmail(email: string): Promise<boolean> {
  const html = `
    <p>You have been unsubscribed from the Neuroklast mailing list.</p>
