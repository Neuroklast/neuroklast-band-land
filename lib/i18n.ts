...
const translations: Record<string, Record<string, string>> = {
  ...
  'cookie.analyticsBasis':     { en: 'Legal basis: Art. 6(1)(a) GDPR (consent) / § 25 TDDDG', de: 'Rechtsgrundlage: Art. 6 Abs. 1 lit. a DSGVO (Einwilligung) / § 25 TDDDG', uk: 'Правовое основание: ст. 6(1)(а) GDPR (согласие) / § 25 TDDDG', it: 'Base giuridica: Art. 6(1)(a) GDPR (consenso) / § 25 TDDDG', es: 'Base legal: Art. 6(1)(a) RGPD (consentimiento) / § 25 TDDDG', pt: 'Base legal: Art. 6.º(1)(a) RGPD (consentimento) / § 25 TDDDG', ja: '法的根拠：GDPR第6条第1項(a)（同意） / § 25 TDDDG', ko: '법적 근거: GDPR 제6조 제1항 (a) (동의) / § 25 TDDDG' },
  'cookie.essentialDesc':      { en: 'Image caching (IndexedDB), locale preference, admin session. Images are loaded via wsrv.nl CDN (IP transmitted). No consent required — Art. 6(1)(f) GDPR / TDDDG § 25(2).', de: 'Bild-Caching (IndexedDB), Spracheinstellung, Admin-Session. Bilder werden über wsrv.nl CDN geladen (IP-Übermittlung). Keine Einwilligung erforderlich — Art. 6 Abs. 1 lit. f DSGVO / § 25 Abs. 2 TDDDG.', uk: 'Кэш изображений (IndexedDB), настройка языка, сессия администратора. Изображения загружаются через CDN wsrv.nl (передача IP). Согласие не требуется — ст. 6(1)(f) GDPR / TDDDG § 25(2).', it: 'Cache immagini (IndexedDB), preferenza lingua, sessione admin. Le immagini vengono caricate tramite CDN wsrv.nl (trasmissione IP). Nessun consenso richiesto — Art. 6(1)(f) GDPR / TDDDG § 25(2).', es: 'Caché de imágenes (IndexedDB), preferencia de idioma, sesión de administrador. Las imágenes se cargan mediante CDN wsrv.nl (transmisión de IP). Sin consentimiento requerido — Art. 6(1)(f) RGPD / TDDDG § 25(2).', pt: 'Cache de imagens (IndexedDB), preferência de idioma, sessão de administrador. As imagens são carregadas via CDN wsrv.nl (transmissão de IP). Sem consentimento necessário — Art. 6.º(1)(f) RGPD / TDDDG § 25(2).', ja: '画像キャッシュ（IndexedDB）、ロケール設定、管理セッション。画像はwsrv.nl CDN経由で読み込まれます（IP転送）。同意不要 — GDPR第6条第1項(f) / TDDDG § 25(2)。', ko: '이미지 캐싱(IndexedDB), 언어 설정, 관리 세션. 이미지는 wsrv.nl CDN을 통해 로드됩니다(IP 전송). 동의 불필요 — GDPR 제6조 제1항 (f) / TDDDG § 25(2).' },
  ...
  'embed.jackIn':              { en: 'JACK IN', de: 'JACK IN', uk: 'JACK IN', it: 'JACK IN', es: 'JACK IN', pt: 'JACK IN', ja: 'JACK IN', ko: 'JACK IN' },
  'embed.jackInAria':          { en: 'Slide to load third-party player', de: 'Schieben zum Laden des Players', uk: 'Проведіть, щоб завантажити плеєр', it: 'Scorri per caricare il player', es: 'Desliza para cargar el reproductor', pt: 'Desliza para carregar o leitor', ja: 'スライドしてプレーヤーを読み込む', ko: '슬라이드하여 플레이어 로드' },
  'youtube.consentHint':       { en: 'Click to load video. This will connect to YouTube (Google).', de: 'Klicken zum Laden des Videos. Dabei wird YouTube (Google) kontaktiert.', uk: 'Натисніть, щоб завантажити відео. Буде з’єднання з YouTube (Google).', it: 'Clicca per caricare il video. Verrà contattato YouTube (Google).', es: 'Haz clic para cargar el vídeo. Se conectará a YouTube (Google).', pt: 'Clica para carregar o vídeo. Será contactado o YouTube (Google).', ja: 'クリックして動画を読み込みます。YouTube（Google）に接続します。', ko: '클릭하여 영상을 로드합니다. YouTube(Google)에 연결됩니다.' },
