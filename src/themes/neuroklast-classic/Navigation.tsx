...
export default function NeuroklastClassicNavigation({
  ...
  return (
    <>
      <motion.nav
        className="fixed top-0 left-0 right-0 z-50 bg-background/75 backdrop-blur-md border-b border-primary/15"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 md:py-4 flex items-center justify-between">
          ...
          <button
            ...
            className={`text-base md:text-lg font-mono tracking-[0.08em] text-foreground hover:text-primary transition-colors touch-manipulation ${glitch ? 'opacity-70 translate-x-px' : ''}`}
            style={{ transition: glitch ? 'none' : 'color 0.2s' }}
          >
            <span className="text-primary/60">&gt;</span>{' '}
            {(siteName || 'NEUROKLAST').toUpperCase()}
          </button>
