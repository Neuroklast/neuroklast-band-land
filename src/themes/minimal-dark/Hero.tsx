  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-background">
      ...
      >
        ...
        <div className="mt-8 font-mono text-xs text-muted-foreground/50 flex flex-wrap items-center justify-center gap-3">
          {genres && genres.length > 0 ? (
            <>
              {genres.map((genre, i) => (
                <span key={i}>
                  {genre.toUpperCase()}
                  {i < genres.length - 1 && <span className="mx-2">•</span>}
                </span>
              ))}
