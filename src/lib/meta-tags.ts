 ...
 * needed for SEO, social sharing, and browser theming.  The caller can either
 * inject the result into the DOM at runtime or use it during build-time HTML
 * generation.
 *
 * Related issue: #160
 */
...
function buildMusicGroupJsonLd(config: SiteConfig): object {
  ...
  const data: Record<string, unknown> = {
    ...
    '@type': config.siteType === 'dj' ? 'MusicGroup' : 'MusicGroup',
    name,
  }
...
/**
 * Generate a complete `MetaTagSet` from a `SiteConfig`.
 *
 * All values fall back to sensible defaults when the relevant config fields
 * are not set, so this function never throws.
 */
...
/**
 * Apply a `MetaTagSet` to the current document (browser only).
 * Creates or updates the relevant `<meta>` / `<title>` / `<link>` elements.
 */
