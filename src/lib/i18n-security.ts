...
const translations: Record<string, Record<Locale, string>> = {
  ...
  'mod.canaryDocumentsTip':       { en: 'Places realistic-looking documents (credentials, DB exports, API keys) in tarpit directories like /admin/backup/. When an attacker downloads and opens them, embedded tracking reveals their real IP, OS, browser, timezone, and screen resolution — even behind proxies (via WebRTC STUN). Additionally, every robots.txt honeypot 403 page embeds canary tracking, so all collected forensic data is stored in the attacker profile.', de: 'Platziert realistisch aussehende Dokumente (Zugangsdaten, DB-Exporte, API-Keys) in Tarpit-Verzeichnissen wie /admin/backup/. Wenn ein Angreifer sie herunterlädt und öffnet, enthüllt eingebettetes Tracking seine echte IP, OS, Browser, Zeitzone und Bildschirmauflösung — selbst hinter Proxies (via WebRTC STUN). Zusätzlich bettet jede robots.txt-Honeypot-403-Seite Canary-Tracking ein, sodass alle forensischen Daten im Angreifer-Profil gespeichert werden.' },
