...
export default function CyberCloseButton({ onClick, label = 'CLOSE', className = '' }: CyberCloseButtonProps) {
  return (
    <motion.button
      className={`group relative flex items-center gap-2 px-3 py-1.5 border border-primary/40 bg-black/60 hover:bg-primary/20 hover:border-primary transition-all duration-200 font-mono text-[10px] tracking-widest text-primary/70 hover:text-primary z-10 ${className}`}
      onClick={onClick}
